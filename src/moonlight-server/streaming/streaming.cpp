  ...
  if (gst_buffer_map(buffer.get(), &map, GST_MAP_READ)) {
    ...
    udp_sink->socket->async_send_to(
        ...
        *udp_sink->client_endpoint,
        [buffer, sample, map_ptr, udp_sink](const boost::system::error_code &error, std::size_t bytes_sent) {
          if (error) {
...
void start_streaming_video(immer::box<events::VideoSession> video_session,
  ...
  run_pipeline(pipeline, [video_session, event_bus, udp_sink, ctx_data_ptr](auto pipeline) {
    ...
    auto pause_handler = event_bus->register_handler<immer::box<events::PauseStreamEvent>>(
        [sess_id = video_session->session_id, rtsp_fake_ip = video_session->rtsp_fake_ip, pipeline](const immer::box<events::PauseStreamEvent> &ev) {
...
void start_streaming_audio(immer::box<events::AudioSession> audio_session,
  ...
  run_pipeline(pipeline, [session_id = audio_session->session_id, rtsp_fake_ip = audio_session->rtsp_fake_ip, udp_sink, event_bus](auto pipeline) {
    if (auto app_sink_el = gst_bin_get_by_name(GST_BIN(pipeline.get()), "wolf_udp_sink")) {
      logs::log(logs::debug, "Setting up wolf_udp_sink");
      g_assert(GST_IS_APP_SINK(app_sink_el));
    ...
    auto pause_handler = event_bus->register_handler<immer::box<events::PauseStreamEvent>>(
        [session_id, rtsp_fake_ip, pipeline](const immer::box<events::PauseStreamEvent> &ev) {
